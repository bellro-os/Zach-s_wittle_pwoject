 ...
 * metered generate both check the session server-side). Gated: the signed-in
 * HUB (/home) and the STUDIO (/comps) require a free account; anonymous
 * visitors are sent to /join with a redirect back. The redirect carries the
 * ORIGINAL query string too, so arrival intent (?demo=1 / ?address= /
 * ?parcelId= / ?intent=) survives the auth wall — safeAuthRedirect re-sanitizes
 * it down to the whitelisted keys on the way back out.
 */
export async function proxy(req: NextRequest) {
  const path = req.nextUrl.pathname;

  const gated =
    path === "/home" || path === "/comps" || path.startsWith("/comps/");
  if (gated) {
