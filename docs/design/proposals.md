...
1. **[SHIPPED 2026-07-16 — mediums batch: true deep-ink .cb-dark, one dark band/page, market-reports+FinalCTA now tint bands]** after the cb-dark light flip (bg == page bg, no
...
2. **[SHIPPED 2026-07-16 — ember border, tint header band, Most popular pill, differentiated CTA]** — reads visually identical to Free.
...
4. **[SHIPPED 2026-07-16 — ONE locked evidence band + header Upgrade only; ProPitchBanner removed]** — collapse the three ZONE-2
   LockedPanels (report-view.tsx:454-465) into one locked band with a single unlock button.
   `[brand/monetization call]`
5. **[SHIPPED 2026-07-16 — rows are same-tab links, row-level onClick removed, Back-to-portfolio chip]** inside the row, including text
...
7. **[SHIPPED 2026-07-16 — demo/address/parcelId/intent survive proxy + sanitizer; first-run suppressed on intent]** and ?demo=1 is stripped by
